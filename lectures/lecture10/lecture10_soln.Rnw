...
\end{itemize}
{\color{blue} The conditions are satisfied.}
\vspace{10pt}
\item Calculate the test statistic.
{\color{blue}
\begin{align*}
t = \frac{\bar{x} - \mu_0}{s / \sqrt{n}}
= \frac{6.8 - 8}{1.4 / \sqrt{40}}
= \frac{-1.2}{1.4 / \sqrt{40}}
= -5.42
\end{align*}
R command:
\begin{verbatim}
...
\vspace{5pt}
{\color{blue}$p$-value $= \texttt{pt(-5.42, df = 39)} = 0.0000017$\\
Since $p$-value $<0.05$, we reject $H_0$.\footnote{{\color{blue} Scientific notation: 
$\texttt{1.7e-06} = 1.7 \cdot 10^{-6} = 0.0000017$}}}\\
\vspace{5pt}
\includegraphics[scale=0.4]{figure/p_value_ex.pdf}

\vspace{10pt}
\item What is the conclusion of the test in the context of the data?\\
\vspace{5pt}
{\color{blue} The data provide evidence that on average Americans get significantly less than 8 hours of sleep on a typical weekday.}
\end{enumerate}
\end{frame}
